...
* Check the `ConnectionStrings` in `appsettings.json` files under the `ProductManagement.AuthServer`, `ProductManagement.HttpApi.Host` and `ProductManagement.DbMigrator` projects and change it if you need.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p 1b213dfd-4984-40c0-93f6-33c52018f20e
```

> `1b213dfd-4984-40c0-93f6-33c52018f20e` is the password of the certificate, you can change it to any password you want.
...
* `ProductManagement.AuthServer`: ASP.NET Core MVC / Razor Pages application that is integrated OAuth 2.0(`OpenIddict`) and account modules. It is used to authenticate users and issue tokens.
